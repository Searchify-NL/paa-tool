...
# Database imports
from sqlalchemy import create_engine, Column, Integer, String, DateTime, Text, Boolean, Float, ForeignKey, Enum, and_
...
class FeatureChange(Base):
    ...
    project_id = Column(Integer, ForeignKey("projects.id"), nullable=False)
    keyword = Column(String(500), nullable=False)
    feature_type = Column(String(50))  # "featured_snippet", "paa", "video", etc.
    action = Column(String(20))  # "gained", "lost"
    competitor = Column(String(255))  # Domain that gained/lost the feature
    timestamp = Column(DateTime, default=lambda: datetime.now(timezone.utc))
...
def schedule_job(project_id: int, frequency: str, run_time: dt_time = dt_time(9, 0)):
    """Schedule a job for a project with minute-level precision."""
    db = get_db()
    try:
        # Build cron expression based on frequency and selected time
        minute = run_time.minute
        hour = run_time.hour
        if frequency == "daily":
            cron_expr = f"{minute} {hour} * * *"  # Every day at specified time
        elif frequency == "weekly":
            # Default to Monday; user can extend later
        ...
        scheduler.add_job(
            ...
            args=[project_id],
            id=f"project_{project_id}_{frequency}_{hour:02d}{minute:02d}",
            replace_existing=True
        )
        
        logger.info(f"Scheduled job for project {project_id} with frequency {frequency} at {hour:02d}:{minute:02d}")
...
def parse_cron_time(cron_expression: str) -> str:
    """Parse cron expression to extract and format time."""
    try:
...
def build_seed_aggregation(qna_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "answered_by_site" in qna_df.columns:
        ...
        # Create answered_by_site column based on answer content
        qna_df['answered_by_site'] = qna_df['answer'].apply(
            lambda x: 1 if len(str(x).strip()) > 10 and not str(x).lower().startswith(("no answer", "not found", "n/a", "none")) else 0
        )
        seed_agg = (
    ...
    if not can_manage:
        ...
        if project_schedules:
            ...
            for schedule in project_schedules:
                ...
                with col1:
                    # Parse cron expression to show user-friendly time
                    time_str = parse_cron_time(schedule['cron_expression'])
                    frequency_display = schedule['frequency'].title()
                    st.write(f"**{frequency_display}** at **{time_str}**")
                with col2:
                    if st.button(f"Run Now", key=f"run_{schedule['id']}"):
                        st.info("🚀 Running scheduled job now...")
                        try:
                            run_scheduled_job(st.session_state.selected_project_id)
        ...
        with st.form("schedule_form"):
            ...
            with col2:
                run_time_input = st.time_input("Run Time (Hour:Minute)", value=dt_time(9, 0), help="Set the exact time including minutes when the job should run")
            
            # Add immediate test option
            test_now = st.checkbox("Test Schedule Now", help="Run the job immediately to test the configuration")
            
            if st.form_submit_button("Create Schedule"):
                if test_now:
                    # Run job immediately for testing
                    st.info("🧪 Testing schedule - running job now...")
                    try:
                        run_scheduled_job(st.session_state.selected_project_id)
                ...
                schedule_job(st.session_state.selected_project_id, frequency, run_time_input)
                st.success(f"✅ Schedule created! Job will run {frequency} at {run_time_input.strftime('%H:%M')}")
                st.rerun()
            ...
            if st.session_state.processed_qna_data is not None:
                ...
                if view_mode == "Graph View":
                    ...
                    if "answered_by_site" in clustered_df.columns:
                        ...
                        # Ensure answered_by_site column exists before groupby
